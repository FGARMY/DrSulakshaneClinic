...
export default function Services() {
  ...
  return (
    <section id="services" className="py-40 px-4 sm:px-6 bg-[#fafafa] dark:bg-[#020617] relative overflow-hidden grain-texture transition-colors duration-500">
      ...
      <div className="absolute inset-0 opacity-[0.02] dark:opacity-[0.05] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#1e1b4b 0.5px, transparent 0.5px)', backgroundSize: '40px 40px' }}></div>
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between mb-32 gap-16 opacity-0 animate-fade-in-up">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-10 h-[1px] bg-indigo-950 dark:bg-indigo-400"></div>
              <span className="text-[11px] font-bold uppercase tracking-[0.4em] text-indigo-950/40 dark:text-indigo-400/60">Clinical Record & Registry</span>
            </div>
            <h2 className="text-5xl md:text-7xl font-display font-bold text-indigo-950 dark:text-slate-100 leading-[0.95] tracking-tight mb-8">
          ...
          <div className="lg:w-[400px] relative">
            <div className="medical-border bg-white dark:bg-slate-900/50 dark:backdrop-blur-xl p-8 clinic-shadow rounded-sm relative dark:border-slate-800">
              ...
              <div className="pt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <div className="text-sm font-bold text-indigo-950 dark:text-indigo-400">Swati Sulakshane, M.S. (Gen. Surgery)</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-slate-100 dark:bg-slate-800 medical-border overflow-hidden rounded-sm shadow-2xl shadow-indigo-950/5 relative dark:border-slate-800">
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-12">
                  <span className="text-[10px] font-bold text-amber-600 uppercase tracking-widest bg-amber-50 dark:bg-amber-900/20 px-3 py-1 rounded-sm opacity-0 animate-fade-in-up" style={{ animationDelay: `calc(${item.delay} + 0.2s)` }}>
                    {item.id}
                  </span>
                  <div className="text-slate-300 dark:text-slate-700 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-all duration-500 w-10 h-10 opacity-0 animate-scale-in group-hover:scale-110" style={{ animationDelay: `calc(${item.delay} + 0.3s)` }}>
                    {item.icon}
                  </div>
                </div>

                <div className="flex-1">
                  <h3 className="text-2xl font-display font-bold text-indigo-950 dark:text-slate-100 mb-6 leading-[1.1] pr-8 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-500 tracking-tight">{item.title}</h3>
                  ...
                  {item.extra && (
                    <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-4">
                      <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-amber-500"></div>
                        <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 dark:text-slate-600">Clinical Protocol</span>
                      </div>
                ...
                <div className="mt-12 flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[9px] uppercase font-bold text-slate-300 dark:text-slate-700 tracking-[0.2em] mb-2 group-hover:text-amber-600 transition-colors">Lead Clinician</span>
                    <span className="text-xs font-serif italic text-slate-900 dark:text-slate-200">{item.expert}</span>
                  </div>
                  <div className="w-10 h-10 border border-slate-100 dark:border-slate-800 flex items-center justify-center text-slate-300 dark:text-slate-700 group-hover:bg-indigo-950 dark:group-hover:bg-white group-hover:text-white dark:group-hover:text-indigo-950 group-hover:border-indigo-950 dark:group-hover:border-white transition-all duration-500 rounded-full">
        ...
        <div className="mt-32 flex flex-col items-center justify-center gap-12 opacity-0 animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
          ...
          <div className="flex flex-wrap justify-center gap-12 text-slate-300 dark:text-slate-700">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] dark:text-slate-600">Board Certified Protocols</span>
            </div>
            <div className="flex items-center gap-3">
              <Activity size={18} className="text-slate-200 dark:text-slate-800" />
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] dark:text-slate-600">Evidence-Based Management</span>
            </div>
          </div>
        </div>
