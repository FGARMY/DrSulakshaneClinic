...
export default function Resources() {
  return (
    <section id="resources" className="py-24 px-4 sm:px-6 bg-white overflow-hidden">
      <div className="container mx-auto max-w-7xl">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-full font-bold text-xs uppercase tracking-widest mb-6">
          ...
          <Link href="#all-resources" className="text-indigo-600 font-bold flex items-center gap-2 hover:translate-x-1 transition-transform group">
            View All Resources
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {RESOURCES.map((item) => (
            <div 
              key={item.id}
              className={`${item.size} group p-8 rounded-[2.5rem] border border-slate-100 hover:border-indigo-100 hover:shadow-[0_20px_50px_rgba(79,70,229,0.1)] transition-all duration-500 flex flex-col justify-between opacity-0 animate-fade-in-up`}
              style={{ animationDelay: item.delay }}
            >
              <div>
                <div className="flex justify-between items-start mb-8">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border-2 transition-transform group-hover:scale-110 ${item.color}`}>
                    {item.icon}
                  </div>
                ...
                <h3 className={`font-display font-bold text-slate-900 leading-tight mb-4 ${item.size.includes('lg:col-span-2') ? 'text-3xl' : 'text-xl'}`}>
                  {item.title}
                </h3>
              ...
              <Link href={`#${item.id}`} className="inline-flex items-center gap-2 text-sm font-bold text-slate-900 hover:text-primary transition-colors mt-auto group/btn">
                Read More
