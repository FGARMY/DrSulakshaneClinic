    ...
    >
      <div className={`sm:w-2/5 relative ${isIndigo ? "bg-indigo-100" : "bg-emerald-100"} min-h-[300px] sm:min-h-full flex items-end justify-center pt-8 overflow-hidden`}>
        ...
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-bold text-slate-800 shadow-sm flex items-center gap-1">
          <Award size={14} className={isIndigo ? "text-indigo-600" : "text-emerald-600"} />
          {experience}
        </div>
      </div>
      <div className="sm:w-3/5 p-8 flex flex-col justify-center">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-display font-bold text-2xl text-slate-900">{name}</h3>
          <div className="flex items-center gap-1 bg-amber-50 text-amber-600 px-2.5 py-1 rounded-md text-xs font-bold">
            <Star size={12} className="fill-amber-500" />
            {rating}
          </div>
        </div>
        <p className="text-sm font-semibold text-slate-500 mb-4">{qualification}</p>
        
        <div className={`inline-flex items-center gap-2 ${isIndigo ? "bg-indigo-50 text-indigo-600" : "bg-emerald-50 text-emerald-600"} px-3 py-1.5 rounded-lg text-sm font-semibold w-fit mb-5`}>
          <Stethoscope size={16} />
          {specialization}
        </div>
